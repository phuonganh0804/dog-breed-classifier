...
def get_pet_labels(image_dir):
    ...
    for filename in filenames:
      ...
      if filename[0] != ".":
        ...
        # If filename doesn't already exist in dictionary add it and it's pet label - otherwise print an error message because indicates duplicate files (filenames)
        if filename not in results_dic:
          low_file_list = filename.lower().split("_")
          pet_label = ""
          for word in low_file_list:
            if word.isalpha():
              pet_label += word + " "
